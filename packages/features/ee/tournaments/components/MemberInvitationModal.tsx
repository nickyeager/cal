...
export default function MemberInvitationModal(props: MemberInvitationModalProps) {
  ...
  const createInviteMutation = trpc.viewer.tournaments.createInvite.useMutation({
    onSuccess(token) {
      copyInviteLinkToClipboard(token);
      trpcContext.viewer.tournaments.get.invalidate();
      trpcContext.viewer.tournaments.list.invalidate();
    },
    onError: (error) => {
  ...
  const copyInviteLinkToClipboard = async (token: string) => {
    ...
    const tournamentInviteLink = `${WEBAPP_URL}/tournaments?token=${token}`;
    const orgInviteLink = `${WEBAPP_URL}/signup?token=${token}&callbackUrl=/getting-started`;

    const inviteLink =
      isOrgInvite || (props?.orgMembers && props.orgMembers?.length > 0)
        ? orgInviteLink
  ...
  return (
    <Dialog
      ...
      <DialogContent enableOverflow type="creation" title="Invite Tournament Participant">
        <div>
                ...
                <Button
                  ...
                  onClick={() =>
                    props.token
                      ...
                      : createInviteMutation.mutate({ tournamentId: props.tournamentId })
                  }
