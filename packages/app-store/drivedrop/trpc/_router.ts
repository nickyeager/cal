...
const driveDropRouter = router({
  players: authedProcedure.query(async ({ ctx }) => {
    const handler = await getHandler("players", () => import("./players.handler"));
    return handler({ ctx });
  }),

  leagues: authedProcedure.query(async ({ ctx }) => {
    const handler = await getHandler("leagues", () => import("./leagues.handler"));
    return handler({ ctx });
  }),
...
export default driveDropRouter;
